...
import { AlertTriangle, MessageSquare, FileText, Users, Clock } from "lucide-react"
...
export function ShelterDashboard({ shelterId }: ShelterDashboardProps) {
  ...
  const handleAddMessage = (reportId: string, messageData: Omit<Message, "id">) => {
    const newMessage = {
      ...messageData,
  ...
  const currentShelter = shelterId ? shelterData[shelterId as keyof typeof shelterData] : null
    ...
    <div className="container mx-auto p-4 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">{shelterName}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{isOnline ? "同期可能" : "同期待機中"}</p>
          </CardContent>
        </Card>
      </div>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="h-64 border rounded-lg p-4 bg-muted/20">
                  <p className="text-muted-foreground text-center">チャット機能は次のフェーズで実装予定</p>
                </div>
      ...
      {/* Report Form Modal */}
      {showReportForm && <ReportForm onClose={() => setShowReportForm(false)} onSubmit={handleNewReport} />}
    </div>
  )
}
