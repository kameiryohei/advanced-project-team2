...
import { Send, Clock, User, Shield, MessageCircle, ArrowLeft } from "lucide-react"
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-xl">報告詳細 - #{report.id}</CardTitle>
        ...
        <CardContent>
          <div className="space-y-3">
            <div>
              <h4 className="font-medium text-sm text-muted-foreground">報告内容</h4>
              <p className="text-sm">{report.details}</p>
            </div>
            <div className="flex gap-6 text-sm">
              <div>
                <span className="font-medium text-muted-foreground">報告者: </span>
                <span>{report.reporter}</span>
              </div>
              {report.attachment && (
                <div>
                  <span className="font-medium text-muted-foreground">添付: </span>
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {messages.length === 0 ? (
                ...
                messages.map((message) => (
                  ...
                  >
                    <div className={`max-w-[70%] ${message.isResponder ? "order-2" : "order-1"}`}>
                      <div
                        className={`rounded-lg p-3 ${
                          message.isResponder ? "bg-card border" : "bg-primary text-primary-foreground"
                        }`}
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <div className="flex items-center gap-1">
                            {message.isResponder ? <Shield className="h-3 w-3" /> : <User className="h-3 w-3" />}
                            <span className="text-xs font-medium">{message.responder}</span>
                          </div>
                          <Badge size="sm" className={`text-xs ${getStatusColor(message.status)}`}>
                            {message.status}
                          </Badge>
                        </div>
            ...
            <div className="border-t pt-4">
              <form onSubmit={handleSubmitMessage} className="space-y-4">
                ...
                <div className="flex justify-end">
                  <Button
                    type="submit"
                    disabled={isSubmitting || !newMessage.trim() || !responderName.trim()}
